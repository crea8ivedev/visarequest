...
class CompletedServiceController extends Controller
{
    ...
    {   
            ...
            {   
                $authId = Auth::user()->id;
                $service = Service::where('agent_id', $authId);
                ...
                return DataTables::of($data)
                    ->addColumn('action', function($data) {
                        $button = '<a  href="#" hrefs="/agent/service/view/'.$data->id.'"  name="view" id="'.$data->id.'" class="btn btn-primary btn-sm rounded-0 edit btn btn-sm btn-clean btn-icon" title="View details"><i class="la la-eye"></i></a>
                        ';
            ...
            return view('backend.agent.completed_service.index', compact('page_title', 'page_description', 'page_breadcrumbs'));
    }
}
